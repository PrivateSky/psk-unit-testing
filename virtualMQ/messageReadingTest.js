...
let stringMessage= function(msg){
    ...
    try{
        ...
        if(index==1){
            setTimeout(()=>{
                utils.httpRequest(newArr[index], stringMessage);
            }, 5000);
        }
        else{
            utils.httpRequest(newArr[index], stringMessage)
...
function test(finish) { 
    utils.createServer((server) => {
        //here we try to post some messages as strings (not valid message)
        utils.httpRequest(newArr[index], stringMessage);

        //here we sent some messages as JSON 
        setTimeout(()=>{
            utils.httpRequest(msgArr[swarmIndex], swarmMessage);
        }, 20000);
